...
{
    public class DomainDAL<T> : IDAL<T> where T : class
    {
        protected readonly FcgDbContext _context;
        private readonly ILogger<DomainDAL<T>> _logger;

        public DomainDAL(FcgDbContext context, ILogger<DomainDAL<T>> logger)
        {
            _context = context;
            _logger = logger;
        }
